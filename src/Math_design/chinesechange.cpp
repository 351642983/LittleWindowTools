...
chinesechange::chinesechange(QWidget *parent):
    QWidget(parent)
{
    //fanghui=new QPushButton("返回选择框");
    lineselect=new QLineEdit;
    queren=new QPushButton("转换所选择文件夹中的所有文件名为拼音");
    pbxuanzhe=new QPushButton("选择");
    lxuanzhe=new QLabel("请选择你要转换的文件夹");
    textedit=new QTextEdit;
    texteditshow=new QTextEdit;
    texteditshow->setReadOnly(true);
    pbzhuanhuan=new QPushButton("转换左边文本框中的文本");
    shouzimu=new QCheckBox();
    shouzimudaxie=new QCheckBox;
    quanbudaxie=new QCheckBox;
    kongge=new QCheckBox;
    lquanbudaxie=new QLabel("全部拼音都为大写");
    lshouzimu=new QLabel("拼音只有首字母");
    lshouzimudaxie=new QLabel("拼音首字母大写");
    lkongge=new QLabel("拼音与拼音之间有空格");
    yinchang=new QPushButton("<<");
    yinchang->setFixedWidth(20);
    wenbenshuru=new QLabel("文本输入框：");
    g_yinchang=false;
    ...
    hlayout_1->addLayout(vlayout_1,1);
    hlayout_1->addLayout(vlayout_3);
    vlayout_3->addStretch();
    vlayout_3->addWidget(yinchang);
    vlayout_3->addStretch();
    hlayout_1->addLayout(vlayout_2);
    vlayout_1->addLayout(hlayout_2);
    vlayout_1->addWidget(lineselect);
    vlayout_1->addLayout(hlayout_11);
    hlayout_11->addStretch();
    hlayout_11->addWidget(pbxuanzhe);
    vlayout_1->addLayout(hlayout_9);
    hlayout_9->addWidget(wenbenshuru);
    vlayout_1->addWidget(textedit);
    vlayout_1->addStretch();

    vlayout_1->addLayout(hlayout_3);
    vlayout_1->addLayout(hlayout_4);
    vlayout_1->addLayout(hlayout_5);
    vlayout_1->addLayout(hlayout_6);
    vlayout_1->addLayout(hlayout_7);
    hlayout_2->addWidget(lxuanzhe);
    hlayout_2->addStretch();

    hlayout_3->addWidget(quanbudaxie);
    hlayout_3->addWidget(lquanbudaxie);
    hlayout_3->addStretch();
    hlayout_4->addWidget(shouzimu);
    hlayout_4->addWidget(lshouzimu);
    hlayout_4->addStretch();
    hlayout_5->addWidget(shouzimudaxie);
    hlayout_5->addWidget(lshouzimudaxie);
    hlayout_5->addStretch();
    hlayout_6->addWidget(kongge);
    hlayout_6->addWidget(lkongge);
    hlayout_6->addStretch();
    mainlayout->addLayout(hlayout_8);
    hlayout_8->addStretch();
    hlayout_8->addWidget(queren);
    hlayout_8->addStretch();
    //hlayout_8->addWidget(fanghui);
    hlayout_8->addStretch();


    vlayout_2->addWidget(texteditshow);
    vlayout_2->addLayout(hlayout_10);
    hlayout_10->addWidget(pbzhuanhuan);
    hlayout_10->addStretch();

    setLayout(mainlayout);
    setFixedSize(sizeHint().width(),sizeHint().height());
    setWindowTitle("中文转换拼音");
    QPalette palette;
    palette=texteditshow->palette();
    palette.setColor(QPalette::Base,Qt::lightGray);
    texteditshow->setPalette(palette);
    texteditshow->setFont(QFont("Microsoft YaHei", 13, QFont::Normal));

    connect(yinchang,SIGNAL(clicked()),this,SLOT(yinchangshow()));
    connect(queren,SIGNAL(clicked()),this,SLOT(querenshow()));
    connect(pbzhuanhuan,SIGNAL(clicked()),this,SLOT(zhuanhuanshow()));
    //connect(fanghui,SIGNAL(clicked()),this,SLOT(fanghuishow()));
    connect(pbxuanzhe,SIGNAL(clicked()),this,SLOT(xuanzheshow()));
...
void chinesechange::yinchangshow()
{
    if(!g_yinchang)
    {
        textedit->hide();
        ...
        pbzhuanhuan->hide();
        yinchang->setText(">>");
        setFixedSize(sizeHint().width()/2,sizeHint().height()/1.5);
    ...
    g_yinchang=!g_yinchang;
}
void chinesechange::querenshow()
{
    if(lineselect->text().isEmpty())
    {
        QMessageBox::about(NULL,"错误",QString("请在选择你要将文件夹里面的文件转换为拼音的文件夹"));
    }
    else
    {
        QMessageBox message(QMessageBox::NoIcon, "注意", QString("你是否想要将选择的文件以%1%2%3%4的模式转化为拼音").arg(shouzimu->isChecked()?"\n只显示首字母":"").arg(shouzimudaxie->isChecked()?"\n首字母大写":"").arg(kongge->isChecked()?"\n拼音之间有空格":"\n拼音之间无空格").arg(quanbudaxie->isChecked()?"\n全部大写\n":"\n"), QMessageBox::Yes | QMessageBox::No, NULL);
        if(message.exec() == QMessageBox::Yes)
        {
            QDir *dir=new QDir(lineselect->text());
            QList<QFileInfo> *fileinfo=new QList<QFileInfo>(dir->entryInfoList());
            int countnum=fileinfo->count();

            for(int i=0;i<countnum;i++)
            {
                QFile::rename(fileinfo->at(i).absoluteFilePath(),fileinfo->at(i).path()+"/"+(shouzimu->isChecked()?GetFirstLetters(fileinfo->at(i).completeBaseName()):GetPinyins(fileinfo->at(i).completeBaseName()))+"."+fileinfo->at(i).suffix());
            }
        }

    }
}
void chinesechange::xuanzheshow()
{
    ...
    if(dir.length() == 0) {
            ...
            lineselect->setText(dir);


    }
}

void chinesechange::zhuanhuanshow()
{
    if(!textedit->toPlainText().isEmpty())
    {

        texteditshow->setText(shouzimu->isChecked()?GetFirstLetters(textedit->toPlainText()):GetPinyins(textedit->toPlainText()));
    }
    else
    {
        QMessageBox::about(NULL,"错误",QString("请在输入框内输入需要转换为拼音的中文内容"));
    }
}
void chinesechange::fanghuishow()
{
    QProcess::startDetached(QCoreApplication::applicationFilePath().remove(QString("_")+QString(VERSION)), QStringList());
    exit(0);
}

bool chinesechange::In(wchar_t start, wchar_t end, wchar_t code)
{
...
char chinesechange::Convert(int n)
{
    /*
     * GB2312码范围 (B0A1-F7FE)
...
QString chinesechange::GetFirstLetter(const QString &src)
{
    wchar_t wchr;
    QString firstLetter;
    ...
    if(src.size() > 0)
    {
        ...
        char c = Convert(wchr);
        int ook=(shouzimudaxie->isChecked()||quanbudaxie->isChecked())?0:32;
        if(c != 0)
        {
            firstLetter.append(c+ook);
            if(kongge->isChecked())
                firstLetter.append(" ");
        }
    }
...
QString chinesechange::GetFirstLetters(const QString &src)
{
...
QString chinesechange::GetPinyins(const QString &text)
{
    ...
    for (int i=0; i<text.length(); i++)
    {
        ...
        pinyins += (kongge->isChecked()?" ":"")+(quanbudaxie->isChecked()?GetPinyin(code):changetype(GetPinyin(code)));
    }
    return pinyins;
}
QString chinesechange::changetype(QString pinyin)
{
    QString ook;
    for(int i=0;i<pinyin.length()&&pinyin.at(i)<='Z'&&pinyin.at(i)>='A';i++)
    {
        if(shouzimudaxie->isChecked()&&i==0)
        {
            ook+=pinyin.at(i);
            continue;
        }
        ook+=QString(*(pinyin.toStdString().c_str()+i)+32);
    }

    return ook;
}

QString chinesechange::GetPinyin(int code)
{
    QString pinyin;
    switch(code)
    {
